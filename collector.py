...
# GitHub Actions-runners hebben soms een kapotte IPv6-route: Python probeert
# dan eerst het IPv6-adres van een host en klapt erop met "Network is
# unreachable", nog vóór er contact is met de server zelf. Dwing daarom
# overal IPv4 af.
import socket
_origineel_getaddrinfo = socket.getaddrinfo
def _alleen_ipv4(host, *args, **kwargs):
    return [ai for ai in _origineel_getaddrinfo(host, *args, **kwargs) if ai[0] == socket.AF_INET]
socket.getaddrinfo = _alleen_ipv4
...
def _get(url, pause=0.5):
    last = None
    for poging in range(6):                       # was 3 — meer ruimte voor een trage runner
        try:
            req = urllib.request.Request(url, headers={"User-Agent": UA, "Accept": "*/*"})
            with urllib.request.urlopen(req, timeout=120) as r:   # was 60
                body = r.read().decode("utf-8", errors="replace")
            time.sleep(pause)
            return body
        except (urllib.error.URLError, urllib.error.HTTPError, TimeoutError) as e:
            last = e
            time.sleep(5 * (poging + 1))           # langere, oplopende pauze tussen pogingen
